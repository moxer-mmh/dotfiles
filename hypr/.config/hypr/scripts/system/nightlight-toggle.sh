#!/bin/bash
# nightlight-toggle.sh — Toggle wlsunset night light
# Part of the Hyprland Space Station setup
#
# Args:
#   toggle     (default) — Enable/disable night light
#   status     — Output JSON for waybar
#   temp-up    — Increase temperature by 200K
#   temp-down  — Decrease temperature by 200K
...
start_wlsunset() {
    ...
    sleep 0.2
    # Using -t (min) and -T (max) set to same value forces a fixed temperature
    wlsunset -t "$TEMPERATURE" -T "$TEMPERATURE" &>/dev/null &
    disown
}
